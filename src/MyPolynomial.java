...
public class MyPolynomial {
	...
	public double evaluate(double x) {
		double acc = 0;
		for (int i = coeffs.length - 1; i < coeffs.length; i--) {
	...
	public MyPolynomial add(MyPolynomial p) {
		...
		double[] coeffsResult = new double[newDegree];
		MyPolynomial biggest;
		if (getDegree() > p.getDegree()) {
			coeffsBiggest = getCoeffs();
			coeffsSmallest = p.getCoeffs();
		} else {
			coeffsBiggest = p.getCoeffs();
			coeffsSmallest = getCoeffs();
		}
